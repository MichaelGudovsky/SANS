...
def load_image(image_path):
    ...
    img = tf.keras.applications.xception.preprocess_input(img)  #changing the image feature extraction model
...
def main(params):
    ...
    top_ans_test = get_top_answers(tests, params)
    ans_to_idx_test = {w: i+1 for i, w in enumerate(top_ans_test)}
    idx_to_ans_test = {i+1: w for i, w in enumerate(top_ans_test)}

    json.dump(idx_to_ans, open('./data/idx_to_ans.json', 'w'))
    json.dump(idx_to_ans_test, open('./data/idx_to_ans_test.json', 'w'))
    # fiter data whose answer is not in top_ans
    trains = filter_data(trains, ans_to_idx)
    tests = filter_data(tests, ans_to_idx_test)    
    ...
    for item in tests:
        ...
        all_test_answer.append(ans_to_idx_test[item['answer']])
    ...
    image_model = tf.keras.applications.Xception(include_top=False,  #changing the image feature extraction model
                                              weights='imagenet')
