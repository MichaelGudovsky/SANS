...
class SAN_LSTM(tf.keras.Model):
    def __init__(self,
        ...
        #                                   stateful=True,
        #                                   recurrent_initializer='glorot_uniform')
        lstm_cells = [tf.keras.layers.LSTMCell(units) for _ in range(3)]###############3
        stacked_lstm = tf.keras.layers.StackedRNNCells(lstm_cells)
        self.lstm_layer = tf.keras.layers.RNN(stacked_lstm)    # tf.keras.layers.RNN ->SimpleRNN

        self.att1 = att_Module(units, dim_att, training=training)
        self.att2 = att_Module(units, dim_att, training=training)
        self.att3 = att_Module(units, dim_att, training=training)
    ...
    def imgModel(self, img):
        ...
        img = tf.nn.tanh(self.img_fc(img)) ######################## tf.nn.tanh
    ...
    def attModel(self, v_img, v_que):
        ...
        img_weights_3, u3 = self.att3(v_img, u2)
        # u1, u2 are combined feature from 2 levels
        # two weights
        return img_weights_1, img_weights_2, img_weights_3, u3##############################

    def call(self, q, img):
        v_img = self.imgModel(img)
        v_que = self.queModel(q)
        img_weights_1,img_weights_2, img_weights_3, u = self.attModel(v_img, v_que)  ####################3
        ...
        # print(output)
        return output,img_weights_1, img_weights_2, img_weights_3##################
